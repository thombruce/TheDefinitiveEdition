...
const search = _debounce(async () => {
  results.value = await $fetch('/.netlify/functions/games', { method: 'POST', body: JSON.stringify({ query: query.value }) })
    ...
    <ul class="menu bg-base-200 join-item" v-show="isActive && treated.length">
      <li v-for="result in treated" :key="result.slug" class="w-full">
        <a @click="populate(result.title)" class="flex flex-row w-full">
          ...
          <div class="flex-initial w-1/3 text-neutral-content truncate">
            {{ result.platforms }}
          </div>
        </a>
      </li>
